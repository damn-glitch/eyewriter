...
def display_keyboard(img, keys):
    color_board = (255, 250, 100)
    for key in keys:
        key_label, center, upper_left, bottom_right = key
        try:
            cv2.putText(img, str(key_label), tuple(map(int, center)), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 100), thickness=3)
            cv2.rectangle(img, tuple(map(int, upper_left)), tuple(map(int, bottom_right)), color_board, thickness=2)
        except Exception as e:
            print(f"Error drawing key: {e}")
            print(f"Key details: label={key_label}, center={center}, upper_left={upper_left}, bottom_right={bottom_right}")
